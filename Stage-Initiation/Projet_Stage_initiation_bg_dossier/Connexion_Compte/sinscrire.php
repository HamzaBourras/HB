<?php 
    ...
    $conn->query($sql);
        ...
        $sql="create table IF NOT EXISTS inscrire ( nom varchar(20), cin varchar(20), email varchar(30), motpass varchar(15), telephone varchar(20)) ";
        $conn->query($sql);

$set=0;   
if(isset($_GET['nom']) && isset($_GET['cin']) && isset($_GET['email'])  && isset($_GET['password']) && isset($_GET['tel'])){
    $set=1;
}

$nonempty=0;
if(!empty($_GET['nom']) && !empty($_GET['cin']) && !empty($_GET['email'])  && !empty($_GET['password']) && !empty($_GET['tel'])){
    $nonempty=1;
}
            ...
            $Tele = $_GET['tel'];
                ...
                $existe=0;
                    ...
                    while($row =$client->fetch_assoc()){
                        ...
                        if( $Nm==$Nom || $Cn==$Cin || $El==$Email || $Ps==$Motpass || $Tele==$Tl){
                            $existe=1; break;
                        }
                    }
                    }
    ...
    $sql="INSERT INTO inscrire (nom,cin,email,motpass,telephone) VALUES ('$Nom','$Cin','$Email','$Motpass','$Tele');";                           
                    }
